...
namespace WistBot.Core.Actions
{
    public class DeleteLinkCallbackAction : IBotAction
    {
        ...
        public DeleteLinkCallbackAction(ITelegramBotClient bot, ItemsService wishListItemsService, LocalizationService localizationService)
        {
            _bot = bot;
        ...
        public async Task ExecuteCallback(CallbackQuery callback, CancellationToken token)
        {
            ...
            {
                Console.WriteLine($"Error DeleteLinkCallbackAction: {ex.Message}");
            }
        }
    }
}
