...
{
    public class SetDoneCallbackAction : IBotAction
    {
        ...
        public string Command => BotCallbacks.SetDone;

        public SetDoneCallbackAction(ITelegramBotClient bot, ItemsService itemsService, LocalizationService localizationService, UsersService usersService)
        {
            _bot = bot;
        ...
        public async Task ExecuteCallback(CallbackQuery callback, CancellationToken token)
        {
            try
            {
                ...
                item.CurrentState = Enums.State.Done;
            ...
            {
                Log.Error(ex, "Error SetDoneCallbackAction");
            }
        }
    }
}
